...
export default function AdminNotices() {
  ...
  const publishNotice = async () => {
    if (!title || !content) { Alert.alert("Error", "সব field পূরণ করো"); return; }
    const res = await fetch(`${BASE_URL}/api/admin/notices`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, content, postedBy: user?.email }),
    });
    if (res.ok) { Alert.alert("Success", "Notice publish হয়েছে"); setModal(false); setTitle(""); setContent(""); fetchNotices(); }
  };
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={styles.cardRow}>
                ...
                <TouchableOpacity onPress={() => deleteNotice(item._id)}><Text style={{ fontSize: 18 }}>🗑️</Text></TouchableOpacity>
              </View>
      ...
      <Modal visible={modal} animationType="slide" transparent>
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>নতুন Notice</Text>
